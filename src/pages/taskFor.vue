      ...
      collection_to_create:[
        {
            ...
            config: {
                property_accesses:[
                        {
                        property_id: "1024929884927544213",
                        access_level: 5
                        }
                    ]
                }
        ...
        {
            owner_type: "subject",
            owner_id: resultCreate?.data?.create_type2?.record?.module?.object?.responsible_id?.object?.id,
            level: 6
        }
    ]
    }
...
try {
    ...
    doneManyRules((result)=>{
        if(result){
            createPremissionForPageWithTasks()
        }
    })
} catch (error) {
...
const createPremissionForPageWithTasks = ()=>{
    const variable={
        input: {
      model_type: "page",
      model_id: "8524350768707752679",
      owner_type: "subject",
      owner_id: newTask.value.executor_id.id,
      level: 3
    }
    }
    console.log("page Variable", variable);
    try {
        createPremissionForTasksPage(variable)
        doneTasksPagePremission((pageresult)=>{
            if(pageresult){
        ...
        console.log("Error create page premision", error);
    }
}
